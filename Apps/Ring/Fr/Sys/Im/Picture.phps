...
class R_Fr_Sys_Im_Picture extends R_Fr_Sys_Creative {
	...
	static public function editGallery( $params )
	{
...
<div class="oo-renderer-title">Новая галерея:</div>
...
<div class="img"><a href="<?=$post->img_full?>"><img
	src="<?=$post->img_preview?>"
	alt="<?=htmlspecialchars( $post->anonce->title . " - " . $post->anonce->description )?>" /></a></div>
...
<div class="img"><a href="<?=$post->url()?>"><img
	src="<?=$post->img_loop?>"
	alt="<?=htmlspecialchars( $post->anonce->title . " - " . $post->anonce->description )?>" /></a></div>
...
<div class="cnt"><a href="<?=$record->url()?>"><img src="<?=$record->creative->img_tiny?>"
	alt="<?=htmlspecialchars( $record->title . " - " . $record->description )?>" /></a></div>
</div>
</div>
<?
	}
	...
	static public function showRssContCallback( O_Dao_Renderer_Show_Params $params )
	{
		$post = $params->record();
		?>
<div class="img"><a href="<?=$post->link()?>"><img
	src="<?=$post->img_preview?>"
	alt="<?=htmlspecialchars( $post->anonce->title . " - " . $post->anonce->description )?>" /></a></div>
<?
