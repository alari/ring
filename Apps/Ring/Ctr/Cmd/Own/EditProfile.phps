...
class R_Ctr_Cmd_Own_EditProfile extends R_Command {
	public function process() {
		...
		if($this->getParam("action") == "upload-ava") {
			if($user->avatar_ext != "-") {
				if(is_file($user->staticFilename("ava.".$user->avatar_ext))) unlink($user->staticFilename("ava.".$user->avatar_ext));
				if(is_file($user->staticFilename("ava-full.".$user->avatar_ext)))unlink($user->staticFilename("ava-full.".$user->avatar_ext));
				$user->avatar_ext = "-";
				$user->save();
			}
