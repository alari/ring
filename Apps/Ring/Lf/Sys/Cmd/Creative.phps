...
class R_Lf_Sys_Cmd_Creative extends R_Lf_Sys_Command {
	...
	public function process()
	{
		if ($this->getParam( "action" ) == "delete") {
			if (!$this->can( "delete", $this->getSite() ))
				throw new O_Ex_AccessDenied( "Вы не можете удалять записи на этом сайте." );
	...
	public function isAuthenticated()
	{
		...
		return $this->can( "read " . $this->instance->system[ "access" ], $this->creative->anonce ) &&
				 $this->can( "read " . $this->creative->anonce[ "access" ], 
